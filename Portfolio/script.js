  ...
  <div class="container">
    <div class="row g-5 align-items-center">
      <div class="col-lg-6 py-6 pb-0 pt-lg-0">
        ...
        <div class="typed-text d-none" id="typedSource">Web Designer, Web Developer, Front End Developer, Apps Designer, Apps Developer</div>
    ...
    <div class="row g-5">
      <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.1s">
        <div class="d-flex align-items-center mb-5">
          ...
          <h3 class="lh-base mb-0" id="aboutHeadline">of working experience</h3>
        </div>
      ...
      <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.5s">
        <div class="row g-3 mb-4" id="aboutImages"></div>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    const payload = { personal, skills, education, experience,professionality, raw: data };
    populateAll(payload);
  } catch (err) {
...
function populateAll({ personal = {}, skills = {}, education = {}, experience = [],professionality = [], raw = {} }, notice) {
  // HERO
  ...
  $("#typedSource").textContent = safe(personal.typed || personal.professions || "Web Designer, Web Developer, Front End Developer");
  $("#playVideoBtn").dataset.src = safe(personal.video || personal.videoUrl || "");
  $("#downloadCV").href = safe(personal.cv || personal.Cv || "#");
// Professionality data display
$("#Years").textContent = safe(professionality.years || "0");
$("#aboutHeadline").textContent = safe(professionality.aboutHeadline || "of working experience as a web designer & developer");
$("#aboutParagraph").textContent = safe(professionality.aboutParagraph || "");
$("#aboutPara2").textContent = safe(professionality.aboutPara2 || "");
$("#aboutPara3").textContent = safe(professionality.aboutPara3 || "");
...
// Optional: display images if needed
if (professionality.ProfilePhoto) $("#profilePhoto").src = professionality.ProfilePhoto;
if (professionality.CnicFront) $("#cnicFront").src = professionality.CnicFront;
if (professionality.CnicBack) $("#cnicBack").src = professionality.CnicBack;

  // About images: if personal.images array or provided keys
  const aboutImagesWrap = $("#aboutImages");
  aboutImagesWrap.innerHTML = "";
  const aboutImages = personal.images || personal.AboutImages || [];
  if (Array.isArray(aboutImages) && aboutImages.length) {
    aboutImages.slice(0,4).forEach(src => {
      const col = document.createElement("div");
      col.className = "col-sm-6";
      col.innerHTML = `<img class="img-fluid rounded" src="${src}" alt="">`;
      aboutImagesWrap.appendChild(col);
    });
  }
...
function displayProfessionality(professionality) {
  ...
  textCol.innerHTML = `
    <h5>${safe(professionality.aboutHeadline || "Professional Experience")}</h5>
    <hr class="text-primary my-2">
  ...
  // SERVICES
  const servicesWrap = $("#servicesList");
  servicesWrap.innerHTML = "";
  const services = personal.services || raw.services || [];
  if (Array.isArray(services) && services.length) {
    services.forEach(s => {
      ...
      col.innerHTML = `
        <div class="service-item d-flex flex-column flex-sm-row bg-white rounded h-100 p-4 p-lg-5">
          ...
          <div class="ms-sm-4">
            <h4 class="mb-3">${safe(s.title || s.name)}</h4>
            <h6 class="mb-3">Start from <span class="text-primary">${safe(s.price || s.start || "$199")}</span></h6>
            <span>${safe(s.description || s.desc || "")}</span>
          </div>
        </div>
      `;
    ...
    // keep default static items intact (if you prefer)
    servicesWrap.innerHTML = document.querySelectorAll("#servicesList > .col-lg-6").length ? "" : `
      <div class="col-lg-6">
        <div class="service-item d-flex flex-column flex-sm-row bg-white rounded h-100 p-4 p-lg-5">
          <div class="bg-icon flex-shrink-0 mb-3">
            <i class="fa fa-crop-alt fa-2x text-dark"></i>
          ...
          <div class="ms-sm-4">
            ...
            <span>Design and brand services.</span>
          </div>
        </div>
      </div>
    `;
...
function initTyped() {
  ...
  const typedSource = $("#typedSource");
  if (!typedOutput || !typedSource) return;
  // if typed.js loaded
  if (window.Typed) {
    ...
    window.__appTyped = new window.Typed('#typedOutput', {
      strings: typedSource.textContent.split(",").map(s => s.trim()),
      typeSpeed: 60,
      backSpeed: 35,
      backDelay: 1600,
      loop: true
    ...
    let i = 0;
    const arr = typedSource.textContent.split(",").map(s => s.trim()).filter(Boolean);
