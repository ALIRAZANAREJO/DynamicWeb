...
// exact helper (Realtime DB safe)
const toSafeEmail = (e) => {
  const [local, domain] = e.split("@");
  return `${local}@${domain.replace(/\./g, "_")}`;
};
...
async function fetchUser() {
  ...
  const snap = await get(
    child(dbRef, `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio`)
  );
...
// BEFORE html2canvas
clone.style.width = '794px';   // desktop cv-sheet width
clone.style.height = '1123px'; // desktop height
...
// Force main and sidebar width in pixels
const sidebar = clone.querySelector('.sidebar');
const main = clone.querySelector('.main');
if (sidebar) sidebar.style.width = '40%';
if (main) main.style.width = '60%';


  // 3️⃣ DISPLAY UNDER PDF FOR VERIFICATION
  const previewContainer =
    document.getElementById('pdfPreviewContainer') || document.createElement('div');
  previewContainer.id = 'pdfPreviewContainer';
  previewContainer.style.width = '210mm';
  previewContainer.style.height = '297mm';
  ...
  if (!document.body.contains(previewContainer)) document.body.appendChild(previewContainer);


  
  console.log('✅ PDF DOM cloned and displayed for verification (A4 layout)');

  // 4️⃣ WAIT FOR ALL IMAGES TO LOAD
  await waitForImages(clone);

  // 5️⃣ HTML2CANVAS → capture CV as image
  const canvas = await html2canvas(clone, {
    scale: 2,             // increase resolution for sharp PDF
    useCORS: true,
  ...
  // 6️⃣ CREATE jsPDF (A4 portrait in mm)
  ...
  // 7️⃣ ADD IMAGE TO PDF (fit full page)
  pdf.addImage(imgData, 'JPEG', 0, 0, 210, 297);

  // 8️⃣ EXPORT PDF BLOB
  const pdfBlob = pdf.output('blob');

  // 9️⃣ UPLOAD TO FIREBASE STORAGE
  const storagePath = `ResumePdf/Portfolio/${email}/1.pdf`;
...
function wireModalAndButtons(){
  ...
  if (displayBtn) {
    displayBtn.addEventListener('click', () => {
      // do nothing – preview disabled
      console.log('Preview disabled (PDF only)');
    });
  }
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener('click', async () => {
      try {
        ...
        // 🧹 HARD CLEANUP
        document.querySelectorAll('.pdf-clone').forEach(el => el.remove());
