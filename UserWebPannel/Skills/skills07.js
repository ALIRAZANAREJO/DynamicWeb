...
/* Load skills data from CSV file */
async function loadSkillsData(csvFilePath) {
  ...
  rows.slice(1).forEach(row => {
    ...
    if (!STEP2_DATA[category]) {
      STEP2_DATA[category] = { icon: "", color: "", subs: {} };
    }

    if (!STEP2_DATA[category].subs[subcategory]) {
      STEP2_DATA[category].subs[subcategory] = { icon: icon, skills: [] };
    }

    STEP2_DATA[category].subs[subcategory].skills.push(skill);
  });
}

// Example usage: Load the CSV file when the script runs
loadSkillsData('/Step2_DATA_compact.csv').then(() => {
  console.log('Skills data loaded:', STEP2_DATA);
  renderFieldsRow(); // Call to render the fields once data is loaded
});
