...
// under Ar Technologies/{Email} with files uploaded to Storage and URLs saved in DB.
// -----------------------------
// ===== OTP-LIKE EMAIL VERIFICATION USING FIREBASE AUTH (ADD HERE) =====
// Firebase v9 imports
import { initializeApp } from "https://www.gstatic.com/firebasejs/12.7.0/firebase-app.js";
import { getAuth, sendSignInLinkToEmail, isSignInWithEmailLink, signInWithEmailLink }
    from "https://www.gstatic.com/firebasejs/12.7.0/firebase-auth.js";
import { getDatabase, ref, set, onValue, get, child, update, remove,push} from "https://www.gstatic.com/firebasejs/12.7.0/firebase-database.js";
...
//     const dbRef = ref(db, "Ar_Technology");
...
async function getJobReferenceData(emailValue) {
...
async function getProfessionalityData(emailValue) {
    ...
    if (frontFile) {
        const refFront = storageRef(storage, `professionality/${emailValue}/CnicFront_${Date.now()}_${frontFile.name}`);
        const snap = await uploadBytes(refFront, frontFile);
        frontUrl = await getDownloadURL(snap.ref);
    }

    if (backFile) {
        const refBack = storageRef(storage, `professionality/${emailValue}/CnicBack_${Date.now()}_${backFile.name}`);
...
// ===== STORE USER EMAIL IN USERS EMAIL LIST (SAFE) =====
// const safeEmailKey = email.value.trim().toLowerCase().replace(/\./g, "_");

// const usersEmailRef = ref(db, "AR_Technologies/Ai/UsersEmail");
// await push(usersEmailRef, {
//     email: safeEmailKey,
//     savedAt: Date.now()
// });
// ===== END STORE USER EMAIL LIST =====

// ===== END STORE USER EMAIL LIST =====

// ===== END STORE USER EMAIL =====

submitBtn.addEventListener('click', async () => {
   showLoader("Saving Data please wait a Sec...");
    if (!email.value) return alert("Please Enter Your Email.");

    // ===== PREVENT DUPLICATE SAVE =====
    const safeEmail = email.value.replace(/\./g, "_");
    const checkRef = ref(db, `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio`);
    const existingSnap = await get(checkRef);

    if (existingSnap.exists()) {
                  hideLoader();

        alert("Dear User You already saved data with this email. Please Recall data and Update it.");
        return; // stop submit process
    }

 // ===== STORE USER EMAIL WITH INDEX =====
const safeEmailKey = email.value.trim().toLowerCase().replace(/\./g, "_");
const usersEmailRef = ref(db, "AR_Technologies/Ai/UsersEmail");

// Get current list
const usersSnap = await get(usersEmailRef);
let nextIndex = 0;

if (usersSnap.exists()) {
    const keys = Object.keys(usersSnap.val()).map(k => parseInt(k)).filter(n => !isNaN(n));
    nextIndex = keys.length ? Math.max(...keys) + 1 : 0;
}

// Save email at next index
await update(ref(db, `AR_Technologies/Ai/UsersEmail/${nextIndex}`), {
    email: safeEmailKey,
    savedAt: Date.now()
});
// ===== END STORE U
    ...
    // ---- continue with rest of your submit code ----
...
if (imageFile) {
  const imageStorageRef = storageRef(storage, `images/${email.value}/${Date.now()}_${imageFile.name}`);
...
if (selfVideoFile) {
    ...
    const videoStorageRef = storageRef(
        storage,
        `selfVideos/${email.value}/${Date.now()}_${selfVideoFile.name}`
    );
...
localStorage.setItem("selectedSkills", JSON.stringify(skills)); // optional: keep localStorage in sync
        ...
        for (const card of eduCards) {
            ...
            for (const file of marksheetFiles) {
                const refFile = storageRef(storage, `education/${email.value}/${type}/marksheets/${Date.now()}_${file.name}`);
            ...
            for (const file of certificateFiles) {
                const refFile = storageRef(storage, `education/${email.value}/${type}/certificates/${Date.now()}_${file.name}`);
                const snapshot = await uploadBytes(refFile, file);
            ...
            extraDocItems.forEach(docItem => {
                ...
                if (docFile) {
                    const fileRef = storageRef(storage, `education/${email.value}/${type}/extra/${Date.now()}_${docFile.name}`);
        ...
        // =========== Experience ===========
        const experienceData = await collectExperiences(email.value);
        const serviceData = await collectServices(email.value);
        const projectData = await collectProjects(email.value);
        const teamData = await collectTeams(email.value);
        ...
        // Get job reference data
        const jobReferenceData = await getJobReferenceData(email.value);

        // Get professionality data
        const professionalityData = await getProfessionalityData(email.value);

        // Store everything under email


        // const newemailRef = ref(db,`AR_Technologies/Ai/Dynamic_Web/Portfolio/${email.value}/Personal`);

const safeEmail = email.value.replace(/\./g, "_");
const newemailRef = ref(db, `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio`);
...
/*
  ...
  - Uploads files to storage under `experiance/{email}/{expIndex}/...`
*/
async function collectExperiences(emailValue) {
  ...
  for (const wrapper of uniqueWrappers) {
    ...
    if (orgIdentityEl && orgIdentityEl.files && orgIdentityEl.files[0]) {
      try {
        ...
        const path = `experiance/${emailValue}/${expIndex}/idcard/${Date.now()}_${file.name}`;
      ...
      for (const nameInputEl of moreNameInputs) {
        ...
        if (fileInputEl && fileInputEl.files && fileInputEl.files[0]) {
          try {
            ...
            const path = `experiance/${emailValue}/${expIndex}/docs/${Date.now()}_${file.name}`;
            const refFile = storageRef(storage, path);
            const snap = await uploadBytes(refFile, file);
...
/*
  ...
  - Uploads files to storage under `servviced/{email}/{serIndex}/...`
*/
async function collectServices(emailValue) {
  ...
  for (const wrapper of uniqueWrappers) {
    ...
    if (orgIdentityEl && orgIdentityEl.files && orgIdentityEl.files[0]) {
      try {
        const file = orgIdentityEl.files[0];
        const path = `servviced/${emailValue}/${serIndex}/idcard/${Date.now()}_${file.name}`;
        const refFile = storageRef(storage, path);
        const snap = await uploadBytes(refFile, file);
        idCardUrl = await getDownloadURL(snap.ref);
      } catch (err) {
    ...
    if (serwisedNameInputs && serwisedNameInputs.length > 0) {
      for (const nameInputEl of serwisedNameInputs) {
        ...
        if (fileInputEl && fileInputEl.files && fileInputEl.files[0]) {
          try {
            const file = fileInputEl.files[0];
            const path = `servviced/${emailValue}/${serIndex}/docs/${Date.now()}_${file.name}`;
            const refFile = storageRef(storage, path);
            const snap = await uploadBytes(refFile, file);
            docFileUrl = await getDownloadURL(snap.ref);
          } catch (err) {
...
async function collectProjects(emailValue) {
  const wrappers = [];
  ...
  for (const wrapper of uniqueWrappers) {
    ...
    if (imageEl && imageEl.files && imageEl.files[0]) {
      try {
        ...
        const path = `projected/${emailValue}/${proIndex}/image/${Date.now()}_${file.name}`;
        const refFile = storageRef(storage, path);
        const snap = await uploadBytes(refFile, file);
    ...
    for (const docRow of docRows) {
      ...
      if (docFileEl && docFileEl.files && docFileEl.files[0]) {
        try {
          const file = docFileEl.files[0];
          const path = `projected/${emailValue}/${proIndex}/docs/${Date.now()}_${file.name}`;
          const refFile = storageRef(storage, path);
          const snap = await uploadBytes(refFile, file);
          docFileUrl = await getDownloadURL(snap.ref);
        } catch (err) {
...
async function collectTeams(emailValue) {
  ...
  for (const wrapper of wrappers) {
    ...
    if (imgEl && imgEl.files[0]) {
      try {
        ...
        const path = `teamed/${emailValue}/${teamIndex}/profile/${Date.now()}_${file.name}`;
        const refFile = storageRef(storage, path);
        const snap = await uploadBytes(refFile, file);
    ...
    for (const nameInput of docNameInputs) {
      ...
      if (fileInput && fileInput.files[0]) {
        try {
          ...
          const path = `teamed/${emailValue}/${teamIndex}/docs/${Date.now()}_${file.name}`;
          const refFile = storageRef(storage, path);
          const snap = await uploadBytes(refFile, file);
...
//     let emailForLink = localStorage.getItem("recallEmail");
//     if (!emailForLink) {
//         emailForLink = prompt("Please confirm your email for verification:");
//     }

//     await signInWithEmailLink(auth, emailForLink, window.location.href);
//     localStorage.removeItem("recallEmail");
//     alert("Email verified! You can now recall your data.");
// }
...
function getEmailPath(emailStr) {
  if (!emailStr) return null;
  const safeEmail = emailStr.trim().replace(/\./g, "_");
  return `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio`;
}
...
function sessionValid(email) {
    return (
        localStorage.getItem("session_email") === email &&
        localStorage.getItem("session_token")
    );
}

// ------------------------------
// RECALL BUTTON CLICK (SECURED)
// ------------------------------
recallBtn.addEventListener("click", async () => {
  showLoader("Recalling Data please wait a Sec...");

    const emailValue = email.value.trim().toLowerCase();
    if (!emailValue) return alert("Please enter your email.");

    const emailKey = safeKey(emailValue);
    const verifiedRef = ref(db, "verifiedEmails/" + emailKey);

    // Check DB if verified before
    const verifiedSnap = await get(verifiedRef);
    const isVerified = verifiedSnap.exists();

    // ---------------------------------------------------
    // CASE 1: Already verified AND session valid → Recall
    // ---------------------------------------------------
    if (isVerified && sessionValid(emailValue)) {
        console.log("✔ Verified + session valid → Direct recall");
        return startRecall(emailValue);
    }

    // ---------------------------------------------------
    // CASE 2: Already verified BUT session NOT valid → OTP
    // ---------------------------------------------------
    if (isVerified && !sessionValid(emailValue)) {
        console.log("⚠ Verified but new device → Sending OTP");
        await sendLink(emailValue);
        return;
    }

    // ---------------------------------------------------
    // CASE 3: Not verified → Send OTP
    // ---------------------------------------------------
    console.log("⚠ New email → Sending OTP");
    await sendLink(emailValue);
});
...
// SEND OTP EMAIL LINK
// --------------------------
async function sendLink(emailVal) {
    ...
    // Always store pending email for correct OTP matching
    localStorage.setItem("pendingEmail", emailVal);
...
// ------------------------------------------
// PAGE LOAD → HANDLE EMAIL VERIFICATION LINK
// ------------------------------------------
window.addEventListener("load", async () => {

    if (!isSignInWithEmailLink(auth, window.location.href)) return;

    // Always restore the correct email
    let emailToVerify = localStorage.getItem("pendingEmail");

    // If user typed manually → still valid but SAVE it again
    if (!emailToVerify) {
        emailToVerify = prompt("For security, enter your email:");
    ...
    // Mark verified in DB (now ALWAYS correct)
    await set(ref(db, "verifiedEmails/" + safeKey(emailToVerify)), true);

    // Create session token
    saveSession(emailToVerify);

    alert("Email verified successfully!");

    // Remove pending email for next user verification
    localStorage.removeItem("pendingEmail");

    // Auto recall after verification
    startRecall(emailToVerify);
});

// ------------------------------------------
// START THE EXISTING RECALL LOGIC (UNCHANGED)
// ------------------------------------------
async function startRecall(emailValue) {
  
    recalledEmail = emailValue; // <-- store recalled email
    console.log("📥 Secure recall started for:", emailValue);


    // Your full existing recall logic begins here:
    // --------------------------------------------
    await recallProfessionality(emailValue);
    await recallPersonalMultiSelect(emailValue);
  ...
  try {
    const snapshot = await get(ref(db, getEmailPath(email.value)));
    if (!snapshot.exists()) return alert("No record found for this email.");
   ...
   // Correct DB path:
    ...
    // ----------------- Skills recall -----------------
    async function recallSkills(emailValue){
      if(!emailValue) return;

      try{
        const snapshot = await get(ref(db, getEmailPath(email.value)));
        if(!snapshot.exists()) return console.log("No record found for this email.");
...
// attachFileChange();
     ...
     // ----------------- Recall Job Reference Data -----------------
        async function recallJobReferenceData(emailValue) {
        ...
        await recallJobReferenceData(emailValue);
...
// ==========================
async function recallProfessionality(emailValue) {
  if (!emailValue) return;

  try {
    const safeEmail = email.value.replace(/\./g, "_");
const profSnap = await get(ref(db, `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio/Professionality`));
...
async function recallPersonalMultiSelect(emailValue) {
  if (!emailValue) return;

  try {
    const safeEmail = emailValue.replace(/\./g, "_");
const personalSnap = await get(ref(db, `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio/Personal`));
...
// ==========================
// ✅ Full recall after email input
// ==========================
const emailValue = email.value?.trim();
if (emailValue) {
  recallProfessionality(emailValue);
  recallPersonalMultiSelect(emailValue);
}
...
//     const emailValue = document.getElementById('email').value; // Get the email input value
//     if (!emailValue) return alert("Please enter a email number.");

//     try {
//         const snapshot = await get(child(ref(db), `resContainer9/${emailValue}`));
//         if (snapshot.exists()) {
//             const data = snapshot.val();
...
//             alert("Data recalled successfully!");
//         } else {
//             alert("No data found for the given email number.");
//         }
//     } catch (error) {
...
//         if (imageFile) {
//             const imageStorageReference = storageRef(storage, `images/${emailValue}/${Date.now()}_${imageFile.name}`);
...
//         // Update data at the path resContainer9/{email}/Personal
//         await update(ref(db, `resContainer9/${emailValue}`), {
...
// ===============================
// Helper: generate Firebase DB path
// ===============================
function getUpdatePath(emailValue, section) {
  if (!emailValue) throw new Error("Email is required for DB path.");
  const safeEmail = emailValue.replace(/\./g, "_");
    return `AR_Technologies/Ai/Dynamic_Web/${safeEmail}/Portfolio/${section}`;
}
...
// ===============================
// Helpe: upload file and return download URL (unchanged)
// ===============================
...
// ===============================
async function updateSectionData(emailValue, sectionName, sectionData) {
  const baseRef = ref(db, getUpdatePath(emailValue, sectionName));

  if (sectionName === "Education" && typeof sectionData === "object") {
    for (const type in sectionData) {
      const typeRef = ref(db, getUpdatePath(emailValue, `Education/${type}`));
...
// ===============================
async function collectEducationData(emailValue) {
  const educationData = {};
  const eduCards = document.querySelectorAll(".edu-card");
  ...
  for (const card of eduCards) {
    ...
    const markPromises = [];
    for (const file of marksheetFiles) {
      markPromises.push(uploadFile(file, `${getUpdatePath(emailValue, `Education/${type}/marksheets`)}/${Date.now()}_${file.name}`));
    }
    ...
    const certPromises = [];
    for (const file of certificateFiles) {
      certPromises.push(uploadFile(file, `${getUpdatePath(emailValue, `Education/${type}/certificates`)}/${Date.now()}_${file.name}`));
    }
      ...
      if (fileInput?.files?.[0]) {
        extraPromises.push(
          (async () => {
            ...
            const url = await uploadFile(f, `${getUpdatePath(emailValue, `Education/${type}/extra`)}/${Date.now()}_${f.name}`);
...
// Collect Experience
// ===============================
async function collectExperienceData(emailValue) {
  ...
  for (const wrapper of wrappers) {
    ...
    if (idFile) {
      idCardUrl = await uploadFile(idFile, `${getUpdatePath(emailValue, `Experience/${expIndex}/idcard`)}/${Date.now()}_${idFile.name}`);
    }
    ...
    for (let d = 0; d < docNameInputs.length; d++) {
      ...
      if (file) {
        docPromises.push(
          (async () => {
            const url = await uploadFile(file, `${getUpdatePath(emailValue, `Experience/${expIndex}/docs`)}/${Date.now()}_${file.name}`);
...
// Collect Service
// ===============================
async function collectServiceData(emailValue) {
  const services = [];
  ...
  for (const w of wrappers) {
    ...
    const idFile = idFileEl?.files?.[0];
    if (idFile) {
      idCardUrl = await uploadFile(idFile, `${getUpdatePath(emailValue, `Service/${idx}/id`)}/${Date.now()}_${idFile.name}`);
    }
    ...
    for (let d = 0; d < docNameInputs.length; d++) {
      ...
      if (file) {
        docPromises.push(
          (async () => {
            const url = await uploadFile(file, `${getUpdatePath(emailValue, `Service/${idx}/documents`)}/${Date.now()}_${file.name}`);
            return { idx: d, entry: { name: docName, url } };
          })()
        );
      } else {
...
// Collect Project
// ===============================
async function collectProjectData(emailValue) {
  const projects = [];
  ...
  for (const w of wrappers) {
    ...
    if (imgFile) {
      imgUrl = await uploadFile(imgFile, `${getUpdatePath(emailValue, `Project/${idx}/image`)}/${Date.now()}_${imgFile.name}`);
    }
    ...
    for (let d = 0; d < docNameInputs.length; d++) {
      ...
      if (file) {
        docPromises.push(
          (async () => {
            const url = await uploadFile(file, `${getUpdatePath(emailValue, `Project/${idx}/documents`)}/${Date.now()}_${file.name}`);
            return { idx: d, entry: { name: docName, url } };
          })()
        );
      } else {
...
// Collect Team
// ===============================
async function collectTeamData(emailValue) {
  const team = [];
  const wrappers = document.querySelectorAll(".tem-row-wrapper");
  let idx = 0;
  for (const w of wrappers) {
    ...
    const imgFile = imgEl?.files?.[0];
    if (imgFile) {
      profileImgUrl = await uploadFile(imgFile, `${getUpdatePath(emailValue, `Team/${idx}/profile`)}/${Date.now()}_${imgFile.name}`);
    }
    ...
    for (let d = 0; d < docNameInputs.length; d++) {
      ...
      if (file) {
        docPromises.push(
          (async () => {
            const url = await uploadFile(file, `${getUpdatePath(emailValue, `Team/${idx}/documents`)}/${Date.now()}_${file.name}`);
            return { idx: d, entry: { name: docName, url } };
          })()
        );
      } else {
...
// ===============================
async function updateJobReferenceData(emailValue) {
    const jobReferenceData = {
        Reference1: {
    ...
    await updateSectionData(emailValue, "JobReference", jobReferenceData);
    console.log("✅ Updated job reference data:", jobReferenceData);
}

async function updateData() {
  const emailValue = email.value?.trim();
  if (!emailValue) return alert("Please enter email before updating.");
  ...
  try {
    ...
    if (selfVideoInput?.files?.[0]) {
      ...
      if (file.size > maxSize) {
        ...
        const videoStorageRef = storageRef(
          storage,
          `${getUpdatePath(emailValue, `Personal/selfVideo`)}_${Date.now()}_${file.name}`
        );
    ...
    if (imageUpload?.files?.[0]) {
      personalData.ImageUrl = await uploadFile(
        imageUpload.files[0],
        `${getUpdatePath(emailValue, `Personal/profile`)}_${Date.now()}_${imageUpload.files[0].name}`
      );
    }

    await updateSectionData(emailValue, "Personal", personalData);

    // 2) Other sections remain untouched
    const educationData = await collectEducationData(emailValue);
    await updateSectionData(emailValue, "Education", educationData);

    const experienceArr = await collectExperienceData(emailValue);
    await updateSectionData(emailValue, "Experience", experienceArr);

    const serviceArr = await collectServiceData(emailValue);
if (serviceArr.length > 0) {
  await updateSectionData(emailValue, "Service", serviceArr);
}

    const projectArr = await collectProjectData(emailValue);
if (projectArr.length > 0) {
  await updateSectionData(emailValue, "Project", projectArr);
}

    const teamArr = await collectTeamData(emailValue);
if (teamArr.length > 0) {
  await updateSectionData(emailValue, "Team", teamArr);
}

    await updateJobReferenceData(emailValue);

    // --- Skills update (old format preserved) ---
    const skillsRef = ref(db, getUpdatePath(emailValue, `Skills`));
    ...
    for (const name in existingByName) {
      if (!selectedByName[name]) {
        ...
        await remove(ref(db, `${getUpdatePath(emailValue, `Skills`)}/${delKey}`));
      }
    }
    ...
    for (const s of selectedSkills) {
      if (existingByName[s.name]) {
        ...
        if (oldItem.percent !== s.percent) {
          await update(
            ref(db, `${getUpdatePath(emailValue, `Skills`)}/${oldItem.key}`),
            { percent: s.percent }
          );
        }
      } else {
        ...
        await update(
          ref(db, `${getUpdatePath(emailValue, `Skills`)}/${newIndex}`),
          { name: s.name, percent: s.percent || "100%" }
        );
      }
    }
    console.log("✅ Skills perfectly synced");

    // --- Professionality update ---
    ...
    const existingProfSnap = await get(ref(db, getUpdatePath(emailValue, `Professionality`)));
    const existingData = existingProfSnap.exists() ? existingProfSnap.val() : {};

    let existingProfArr = [];
    if (existingData.profession) existingProfArr = existingData.profession.split(",").map(p => p.trim());
    ...
    profData.CnicFront = frontFile
      ? await uploadFile(frontFile, `${getUpdatePath(emailValue, `Professionality/CnicFront`)}_${Date.now()}_${frontFile.name}`)
      : (existingData.CnicFront || "");

    profData.CnicBack = backFile
      ? await uploadFile(backFile, `${getUpdatePath(emailValue, `Professionality/CnicBack`)}_${Date.now()}_${backFile.name}`)
      : (existingData.CnicBack || "");

    profData.ProfilePhoto = profileFile
      ? await uploadFile(profileFile, `${getUpdatePath(emailValue, `Professionality/ProfilePhoto`)}_${Date.now()}_${profileFile.name}`)
      : (existingData.ProfilePhoto || "");

    await updateSectionData(emailValue, "Professionality", profData);
...
document.getElementById("updateBtn")?.addEventListener("click", () => {
    showLoader("Updating Data please wait a Sec...");

    const inputEmail = email.value?.trim().toLowerCase();

    // Block if no recall happened
    if (!recalledEmail) {
        return alert("Please first recall your data and then update it.");
    }

    // Block if recall email ≠ input email
    if (recalledEmail.toLowerCase() !== inputEmail) {
        return alert("Please first recall your data and then update it.");
    }
...
// async function updateData() {
//   const emailValue = email.value.trim();
//   if (!emailValue) return alert("Please enter email first.");

//   const userRef = ref(db, `AR_Technologies/Ai/Dynamic_Web/${emailValue}/Portfolio/${emailValue}/Personal`);
...
//     if (imageUpload.files[0]) {
//       const imgRef = storageRef(storage, `AR_Technologies/Ai/Dynamic_Web/${emailValue}/Portfolio/${emailValue}/profile_${Date.now()}.jpg`);
...
//   try {
//     const userRef = doc(db, "users", emailValue);
...
// ----------------- Delete entire email node -----------------
// deleteBtn.addEventListener('click', async () => {
//     const emailValue = email.value;
//     if (!emailValue) return alert("Please enter a email number.");

//     if (!confirm(`Delete all data for email ${emailValue}? This cannot be undone.`)) return;

//     try {
//         await remove(ref(db, `AR_Technologies/Ai/Dynamic_Web/${emailValue}/Portfolio`));
...
if (isSignInWithEmailLink(auth, window.location.href)) {
    ...
    if (!emailForLink) {
        emailForLink = prompt("Confirm your email:");
    }

    await signInWithEmailLink(auth, emailForLink, window.location.href);
    alert("Email verified! You may now recall your data.");
}
