...
function clearForm() {
    ...
    // Clear education forms similarly...
}
...
function handleFilePreview(input, fileUrl = "") {
  ...
  const wrapper = input.closest(".input-wrapper,.exp-group,.float-card,.ser-group, .pro-group, .tem-group");
  if (!wrapper) return;
...
recallBtn.addEventListener('click', async () => {
  ...
  function handleFilePreview(input, fileUrl = "") {
    ...
    if (file) {
      if (file.type.startsWith("image/")) {
        ...
        preview.src = "https://cdn-icons-png.flaticon.com/512/337/337946.png"; // pdf icon
      } else {
        preview.src = "";
      }
    } else if (fileUrl) {
  ...
  // Helper: attach preview change listener to all file inputs
...
// Restore saved professions into custom tag UI
if (prof.profession) {
  restoreProfessionTags("professionInput", prof.profession);
} else {
  // Clear any previous tags if no data
...
//       console.log("Professionality data recalled successfully ✅");
//       alert("Data recalled successfully!");
//     } else {
...
// ======== Restore Profession Tags Function ========
function restoreProfessionTags(inputId, professionData) {
  const input = document.getElementById(inputId);
  const selectedContainer = input.parentNode.querySelector(".selected-container");
  if (!selectedContainer) return;

  // Convert stored value (comma string or array) into array
  let professions = [];
  if (typeof professionData === "string") {
    professions = professionData.split(",").map(p => p.trim()).filter(p => p);
  } else if (Array.isArray(professionData)) {
  ...
  professions.forEach(item => {
    ...
    removeBtn.onclick = () => {
      tagd.remove();
      // update input value
      const allTags = Array.from(selectedContainer.querySelectorAll(".tagd")).map(t => t.textContent.replace("×", "").trim());
      input.value = allTags.join(", ");
    };
    tagd.appendChild(removeBtn);
    selectedContainer.appendChild(tagd);
  });

  // Update hidden input value for Firebase saving
  input.value = professions.join(", ");
};
...
window.handleFilePreview = function(input, fileUrl = "") {
  document.querySelectorAll('input[type="file"][data-url]').forEach((input) => {
    const url = input.getAttribute("data-url");
    if (url) window.handleFilePreview(input, url);
  });
};
